...
import { Calendar, Clock, MapPin, Copy, Eye, Pencil, Users, ListChecks, CheckCircle2, SkipForward, PlayCircle } from 'lucide-react';
...
export default function EventOverviewPage({ params }: { params: { id: string } }) {
  ...
  async function handleStartEvent() {
    ...
    setIsStarting(false);
    if (response.ok) {
      toast({ title: 'Evento iniciado! A fila já está aberta para inscrições.', variant: 'success' });
      mutate(`/api/events/${event.id}`);
    } else {
      toast({ title: 'Não foi possível iniciar o evento', variant: 'error' });
    }
  }
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            {(event.status === 'DRAFT' || event.status === 'PAUSED') && (
              <Button variant="accent" size="sm" onClick={handleStartEvent} isLoading={isStarting}>
                <PlayCircle className="h-4 w-4" />
                {event.status === 'DRAFT' ? 'Iniciar evento agora' : 'Retomar fila'}
              </Button>
            )}
            <Button
              variant="outline"
